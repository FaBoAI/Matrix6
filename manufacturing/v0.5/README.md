...
JLCPCBのGerber解析では4層・40.64 × 61.00 mmと認識されました。これは製造指示JPG追加前の解析確認です。JPGを追加した現行ZIPはログイン後に再アップロード済みで、PCBAのBOM/CPL入力画面まで進みました。ログイン後の製造条件・金額の再確認、詳細なGerber Viewer確認が残っています。注文は未実施です。
...
ログイン後、次のJLCPCB形式ファイルをPCBAの入力欄へアップロードしました。

- [Matrix6-v0.5-BOM.csv](assembly/Matrix6-v0.5-BOM.csv)：22行にまとめた32部品。14品目のJLCPCB番号とメーカー品番を記載。未選定品は仕様を記載し、JLCPCB番号を空欄にして照合対象にしています。
- [Matrix6-v0.5-CPL.csv](assembly/Matrix6-v0.5-CPL.csv)：32行、全てTop、mm、Gerberと同じ絶対原点。H1/H2は1番ピンの座標から部品中心へ24.13 mm補正（Mid Y = −129.13 mm）。角度はKiCadの値を0〜359度に正規化しています。
- [変換検証](assembly/export-validation.json)：CAD・元CSVのハッシュ、32部品の参照番号一致、中心補正、未確定品目を記録。

これらは**部品照合・見積もり用**で、製造承認済みデータではありません。JLCPCB側のモデルとの回転・極性照合、在庫確認、未選定メーカー品番の確定が必要です。再生成は `python3 scripts/export_jlcpcb_assembly.py`。

修正後の照合画面は22品目中21品目に候補、1品目（H1/H2）が未選定となりました。[アップロード結果と残件](assembly/README.md)を参照してください。

初回の自動照合では、C5（指定1µF/16V/X7R）にX5R品が割り当てられました。仕様に合うSamsung CL10B105KO8NNNC / C59782に修正しています。抵抗4種は、0603・1%・100mWのUNI-ROYAL品を指定して在庫不足の自動候補を置き換えました。F1=C25415531、L1=C780206、R11=C844915、SW1/SW2=C231329も設計指定品番との一致を確認して追記しています。

現在のPCBA画面には実装枚数2枚の表示があり、上記の「基板5枚」という仮見積もりとは区別が必要です。最終発注前に基板枚数・実装枚数・納期・配送条件を揃えて確認します。
